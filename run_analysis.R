...
# Load data frame features and select rows for mean and standard deviation only
features <- read.table("UCI HAR Dataset/features.txt", stringsAsFactors = FALSE)
names(features) <- c("feat_id", "feat_desc")
feat_selected <- filter(features, grepl("mean\\(\\)", feat_desc) | grepl("std\\(\\)",feat_desc))
...
names(data_step4) <- c("activity", "subject", feat_selected$feat_desc)

# Step 5

# Based on data_step4, create a second, independent tidy data set 
# with the average of each variable for each activity and each subject
data_step5 <- group_by(data_step4, activity, subject) %>% summarise_each(funs(mean))

# Set descriptive variable names for the measurement columns (add a prefix "average-")
nc <- ncol(data_step5)
meas_col_names <- paste("average", names(data_step5)[3:nc], sep = "-")
